...
class DataValidation:
    ...
    def validate_all_columns(self, file_name) -> bool:
        try:
            ...
            for col, exp in data_dtype.items():
                if col not in expected_dtype.keys():
                    ...
                    shutil.copy(self.config.unzip_data_dir, self.config.failed_data_dir)
                ...
                if  str(data_type) != str(exp):
                    validation_status = False
                    shutil.copy(self.config.unzip_data_dir, self.config.failed_data_dir)
                    logger.info(f"Data Type mismatch in dataset at {self.config.unzip_data_dir} and moved to {self.config.failed_data_dir}")
                    break
                    
                else:
                    validation_status = True
            if validation_status == True:
                validated_file = f"valid_data_{file_name}"
                data.to_csv(validated_file, index=False)
                shutil.move(validated_file, self.config.valid_data_dir)
